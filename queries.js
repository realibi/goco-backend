...
const loadCallCenterInfo = (request, response) => {
    pool.query('SELECT id, center_category_id, call_center_user_id, center_name, contact_name, center_phone, center_email, first_call_date, first_call_time, first_call_comment, kp_send_date, second_call_date, second_call_time, second_call_comment, meeting_date, meeting_time, saller_user_id, meeting_comitted, meeting_comment, will_conclude_contract, contract_signing_start_date, data_collection_start_date, contract_send_date, contract_send_comment, contract_agreed, contract_agreement_comment, contract_signed, contract_signed_comment, contract_sign_date, operation_personal_user_id FROM public.crm where center_name is not null', (error, results) => {
        if (error) {
            throw error
        }
...
const updateCallCenterRow = (request, response) => {
    ...
    if(meetingDate !== null && meetingTime !== null){
        pool.query('UPDATE public.crm SET center_name=$2, contact_name=$4, center_phone=$3, center_email=$5, first_call_date=$11, first_call_time=$12, first_call_comment=$7, kp_send_date=$6, second_call_date=$14, second_call_time=$13, second_call_comment=$8, meeting_date=$9, meeting_time=$10 WHERE id=$1',
            [
                id,
                companyName,
                phone,
